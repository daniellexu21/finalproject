...
saveRDS(flight_23_full, file = "flight_23_full.rds")
...
flight_24_full <- flight_24 |>
  ...
  mutate(weather = case_when(weather_delay == 0 ~ "0",
                              weather_delay > 0 ~ "1"),
         carrier = case_when(carrier_delay == 0 ~ "0",
                              carrier_delay > 0 ~ "1"),
         prcp = if_else(is.na(prcp), 0, prcp),
         awnd = if_else(is.na(awnd), 0, awnd))|>
  filter(weather != "NA")
...
saveRDS(flight_24_full, file = "flight_24_full.rds")
...
rf_imp_plot
ggsave("variable_importance_rf.png", plot = rf_imp_plot, width = 8, height = 6, dpi = 300)
